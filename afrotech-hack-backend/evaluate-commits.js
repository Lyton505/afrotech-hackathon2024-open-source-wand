...
const checkUser = async function (username) {
  ...
  const getUserRepos = async function (username) {
    try {
      ...
      console.log("Largest repo: ", largestRepo.name);
   ...
   */
  const iterator = (username, repoName) => octokit.paginate.iterator('GET /repos/{owner}/{repo}/commits', {
    owner: username,
    repo: repoName,
    per_page: 2
  });
    ...
    try {
      ...
      if (username && repoName) {
        for await (const response of iterator(username, repoName)) {
          ...
          console.log("commit_msg1: ", commit_msg1);
          console.log("commit_msg2: ", commit_msg2, "\n");

          if (response.data.length > 0) commits.push(response.data[0]);
          if (response.data.length > 1) commits.push(response.data[1]);
          await compareCommitMessages(commit_msg1, commit_msg2);
        ...
        console.log("Username or repoName not set");
      }
    } catch (error) {
